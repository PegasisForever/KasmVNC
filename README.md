...
### Pre-built Package (Ubuntu Jammy / Zorin OS 17, x86_64)

This fork provides pre-built packages that share your existing display `:0` via a web browser on port 6901, with Tailscale Serve support.

#### Option A — .deb package (recommended)

```sh
wget https://github.com/PegasisForever/KasmVNC/releases/latest/download/kasmvncserver_jammy_1.3.4-pegasis_amd64.deb
sudo apt-get install ./kasmvncserver_jammy_1.3.4-pegasis_amd64.deb
```

#### Option B — tar.gz with install script
...
Both options will set up and enable systemd services (`kasmvnc`, `kasmxproxy`, `tailscale-serve`) and add an autostart entry to fix black screen after login.
